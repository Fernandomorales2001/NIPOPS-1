    ...
    categoria.save((err,categoria_save)=>{
        if (err) {
            ...
            if (categoria_save) {
                ...
                res.status(403).send({msg: 'La categoria no se pudo registrar'});
            }
        }
    });
}
    ...
    Categoria.findById({_id: id}, (err,categoria_data) => {
        if (err) {
            ...
            if (categoria_data) {
                ...
                res.status(403).send({msg: 'La categoria no existe'});
            }
        }
    });
}
    ...
    Categoria.findByIdAndRemove({_id:id},(err,categoria_delete)=>{
        if (err) {
            ...
            if (categoria_delete) {
                ...
                res.status(403).send({msg: 'La categoria no se pudo eliminar'});
            }
        }
    });
}
    ...
    Categoria.find({titulo: new RegExp(nombre,'i')},(err,categoria_listado)=>{
        if (err) {
            ...
            if (categoria_listado) {
                res.status(200).send({categoria: categoria_listado});
            } else {
                res.status(403).send({msg: 'La categoria no se pudo eliminar'});
            }
        }
